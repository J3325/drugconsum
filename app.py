...
try:
    ...
    # Preprocessing Data
    df["Target_Cannabis"] = df["Cannabis"].apply(
        lambda x: 1 if x not in ["CL0", "CL1"] else 0
    )
    fitur_zat = [
        "Alcohol",
        "Amphet",
    ...
    st.sidebar.write(
        "Sesuaikan parameter kepribadian di bawah ini untuk melihat hasil keputusan model secara langsung:"
    )
    ...
    sim_escore = st.sidebar.slider(
        "Ekstraversi (Kemampuan Bersosialisasi)", -3.5, 3.5, 0.0
    )
    sim_oscore = st.sidebar.slider(
        "Keterbukaan terhadap Pengalaman Baru", -3.5, 3.5, 0.0
    )
    sim_ascore = st.sidebar.slider("Keramahan Sosial (Agreeableness)", -3.5, 3.5, 0.0)
    sim_cscore = st.sidebar.slider(
        "Kedisiplinan Diri (Conscientiousness)", -3.5, 3.5, 0.0
    )
    ...
    # Menggabungkan Input ke DataFrame Evaluasi Sementara
    input_sim = pd.DataFrame(
        [
            {
                "Age": sim_age,
                "Gender": sim_gender_val,
                "Education": sim_edu,
                "Country": 0.2,
                "Ethnicity": -0.1,
                "Nscore": sim_nscore,
    ...
    input_sim_scaled = scaler.transform(input_sim)

    # Eksekusi Kalkulasi Prediksi Real-Time Simulator
    sim_pred_class = model_dt.predict(input_sim_scaled)
    sim_pred_proba = model_dt.predict_proba(input_sim_scaled)
    sim_pred_anomaly = model_if.predict(input_sim_scaled)
    ...
    **Kesimpulan Utama Analisis Dasbor (Executive Summary):**
    ...
    1. **Prediksi Akurat (~78%):** Algoritma Klasifikasi membuktikan bahwa profil psikologis kepribadian seseorang (terutama tingkat neurotisisme dan pencarian sensasi) memiliki korelasi kuat untuk mendeteksi risiko individu menjadi pengguna zat adiktif.
    2. **Deteksi Kelompok Ekstrem (5%):** Algoritma Deteksi Anomali berhasil memisahkan secara tepat 95 orang yang memiliki rekam jejak penyimpangan perilaku paling ekstrem dibandingkan dengan pola normal 1.790 responden lainnya di dalam dataset.
    """)
    ...
    with col_s1:
        if sim_pred_class[0] == 1:
            st.markdown(
                f"<div style='background-color: #FADBD8; padding: 15px; border-radius: 5px; border-left: 5px solid #E74C3C;'><b>Hasil Prediksi Klasifikasi:</b> Berisiko Tinggi (User)<br>Probabilitas Keyakinan Model: {sim_pred_proba[0][1] * 100:.2f}%</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f"<div style='background-color: #D4EFDF; padding: 15px; border-radius: 5px; border-left: 5px solid #2ECC71;'><b>Hasil Prediksi Klasifikasi:</b> Berisiko Rendah (Non-User)<br>Probabilitas Keyakinan Model: {sim_pred_proba[0][0] * 100:.2f}%</div>",
                unsafe_allow_html=True,
            )
    ...
    with col_s2:
        if sim_pred_anomaly[0] == -1:
            st.markdown(
                "<div style='background-color: #FDEBD0; padding: 15px; border-radius: 5px; border-left: 5px solid #E67E22;'><b>Status Pola Perilaku:</b> Terdeteksi Anomali (Outlier)<br>Kombinasi skor psikologis ini dinilai ekstrem atau sangat langka dalam populasi.</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                "<div style='background-color: #EBF5FB; padding: 15px; border-radius: 5px; border-left: 5px solid #3498DB;'><b>Status Pola Perilaku:</b> Normal (Inlier)<br>Kombinasi skor psikologis berada pada batasan wajar persebaran data umum.</div>",
                unsafe_allow_html=True,
            )
    ...
    # ==========================================
    tab_klasifikasi, tab_anomali = st.tabs(
        ["Model Prediksi Klasifikasi Risiko", "Sistem Deteksi Anomali Perilaku"]
    )
    ...
    with tab_klasifikasi:
        ...
        st.help("""
        **Fungsi Utama Model Ini:** Menjawab pertanyaan 'Apakah seseorang cenderung menjadi pengguna berdasarkan sifat psikologisnya?'.
        Model dilatih menggunakan porsi data latih untuk memetakan aturan klasifikasi biner otomatis (User versus Non-User).
        """)

        col_m1, col_m2 = st.columns([1, 1])
        with col_m1:
            st.metric(
                label="Akurasi Pengujian Model (Ketetapan Prediksi)",
                value=f"{accuracy_score(y_test, y_pred) * 100:.2f}%",
            )

            # Format tabel klasifikasi
            report_dict = classification_report(
                y_test, y_pred, target_names=["Non-User", "User"], output_dict=True
            )
        ...
        with col_m2:
            ...
            sns.heatmap(
                ...
                yticklabels=["Aktual Non-User", "Aktual User"],
                ax=ax1,
            )
            plt.tight_layout()
            st.pyplot(fig1)
            st.caption(
                "**Insight Grafik:** Fokus pada jalur warna diagonal gelap. Dominasi angka tinggi pada diagonal membuktikan tingkat kesalahan prediksi model berada pada kategori rendah."
            )
    ...
    with tab_anomali:
        ...
        st.help("""
        **Fungsi Utama Model Ini:** Mengisolasi objek/responden minoritas yang memiliki kombinasi sifat kepribadian serta riwayat konsumsi lintas zat yang sangat tidak wajar (ekstrem) dibanding mayoritas masyarakat umum.
        """)
        ...
        with col_a1:
            st.metric(
                label="Subjek Berperilaku Wajar (Inliers)",
                value=f"{normal_count} Orang",
            )
        with col_a2:
            st.metric(
                label="Subjek Terdeteksi Ekstrem (Outliers)",
                value=f"{anomali_count} Orang",
            )
        with col_a3:
            st.metric(
                label="Rasio Kontaminasi Penyimpangan",
                value=f"{(anomali_count / total_data) * 100:.2f}%",
            )
        ...
        with col_g1:
            ...
            ax3.scatter(
                X_pca[df_numeric["Anomaly_Label"] == 1, 0],
                X_pca[df_numeric["Anomaly_Label"] == 1, 1],
                color="#3498DB",
                alpha=0.4,
                label="Normal (Pola Umum)",
            )
            ax3.scatter(
                X_pca[df_numeric["Anomaly_Label"] == -1, 0],
                X_pca[df_numeric["Anomaly_Label"] == -1, 1],
    ...
    st.error(
        "Berkas data 'drug_consumption.data' tidak ditemukan dalam repositori sistem. Pastikan letak folder penyimpanan berkas sudah selaras."
    )
